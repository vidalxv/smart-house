...
public class Batedeira implements Eletrodomesticos{
    ...
    public Batedeira(boolean isDesligado, int nivel, int horas) {
    ...
    @Override
    public void status() {
        System.out.println("BATEDEIRA ESTA: " + (getIsDesligado() ? "DESLIGADA" : "LIGADA"));
    }

    @Override
    public double potencia() {
        double potencia;
        if (getNivel() >= 1 && getNivel() <= 6) {
            potencia = 300 / 6 * getNivel();
        } else {
            setNivel(1);
            potencia = 50;
        }
