...
public class Geladeira implements Eletrodomesticos {
    ...
    public Geladeira(boolean isDesligado, int nivel, int horas) {
    ...
    @Override
    public void status() {
        System.out.println("GELADEIRA ESTA: " + (getIsDesligado() ? "DESLIGADA" : "LIGADA"));
    }

    @Override
    public double potencia() {
        double potencia;
        if (getNivel() >= 1 && getNivel() <= 8) {
            potencia = 400 / 8 * getNivel();
        } else {
            setNivel(1);
            potencia = 50;
        }
